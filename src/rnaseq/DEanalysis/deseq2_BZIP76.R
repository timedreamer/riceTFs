...
# Rice gene: NTL3 (LOC_Os01g15640).
# Paper: https://www.ncbi.nlm.nih.gov/pubmed/31733092

# Note: RNA isolated from caryopses of the WT, osbzip76-1 and osbzip76-2 
# at 2 DAF was used for RNA-Seq. 
# We set up three biological replicates for WT and two replicates for 
# osbzip76-1 and osbzip76-2, two independent mutant alleles of OsbZIP76.
...
## manually change
lib_table <- lib_table %>% select(Run, tissue, Genotype) %>% 
  mutate(genotype = factor(str_replace(.$Genotype, "-", "_"), 
                           levels = c("WT","osbzip76_1", "osbzip76_2")))
...
resultsNames(dds)
# de_gene <- results(dds, contrast = c(-1,1,0), tidy = T,
#                 alpha = p_cutoff) %>% filter(padj < p_cutoff)

de_gene1 <- results(dds, contrast = c("genotype", "osbzip76_1", "WT"), tidy = T,
                   alpha = p_cutoff) %>% filter(padj < p_cutoff)

de_gene2 <- results(dds, contrast = c("genotype", "osbzip76_2", "WT"), tidy = T,
                    alpha = p_cutoff) %>% filter(padj < p_cutoff)

# 4. Save result --------------------------------------------------------------------
file_name1 <- paste0("rice_", gene_name, "_", loc_id, "_lof_DESeq2_", nrow(de_gene1), 
                    "_osbzip76-1_vs_wt",".tsv")

file_name2 <- paste0("rice_", gene_name, "_", loc_id, "_lof_DESeq2_", nrow(de_gene2), 
                     "_osbzip76-2_vs_wt",".tsv")

write_tsv(de_gene1, here("result", "rnaseq","DE_result", file_name1))
write_tsv(de_gene2, here("result", "rnaseq","DE_result", file_name2))
