...
# Note: See https://github.com/timedreamer/riceTFs/issues/2 for issues with this dataset.
# In the end , I analzyed the dataset as single-end. The PCA and result looks fine. 
...
## manually change
lib_table <- lib_table %>% select(Run, treatment) %>% 
  arrange(Run) %>% distinct(treatment, .keep_all = T) %>% 
  separate(treatment, sep="_", into=c("genotype","treatment","rep")) %>% 
  mutate(condition = paste0(genotype,"_",treatment)) %>% 
  mutate(condition = factor(condition, levels = c("NIP_control", "NIP_heat",
                                                  "ntl_control", "ntl_heat")))
